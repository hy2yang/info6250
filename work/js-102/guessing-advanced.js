...
function thinkAbout( wordInfo ) {
  ...
  // EDIT BELOW THIS
  let arr=Object.assign([], wordInfo.allWords);     // copy allWord to magic
  return arr;
  // EDIT ABOVE THIS
}
...
function pickGuess( wordInfo, history ) {
  ...
  let guess, temp;
  let min=5;
  if (history.results[history.count]){                        // choose one with least common letters of last guess
    for (let i of wordInfo.magic){
      temp=common(i, history.results[history.count].guess);
      if (temp<min){
        guess=i;
        min=temp;
      }
  ...
  else guess = random(wordInfo.magic);
...
function compareLetters( guess, wordInfo ) {
  ...
  updateCandidates(guess, wordInfo.magic, result.similar);
